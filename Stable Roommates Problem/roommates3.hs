...
main = do
       ...
       let (stable, persons, prefs) = solve inpStr  in do
			solved <- output outh persons prefs
			putStr (if not solved then "" else (if stable then  "Found stable matching\n"
                                               else "ERROR: Matching is unstable.\n"))
       hClose inh
       hClose outh
...
solve str =
   let prefs    = map words (lines str)
       ...
       stable = if null (result(head univ)) then False 
                   else null [p | p <-univ, any (p `elem`) (map wish (wish p))]
...
output:: Handle ->  [Person] -> PrefFn -> IO Bool
output h [] pref = do return True
output h (p:ps) pref
